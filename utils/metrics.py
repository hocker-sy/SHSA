...
class Evaluator:
	...
	def eval(self, model, i2t_metric=True):
		...
		# 	similarity = (similarity + similarity_tws.cuda()) / 2
		...
		if "tws" in model.args.loss_names:
			...
			sims_dict["TWS"] = similarity_tws
			similarity = similarity_tws

		if "sdm" in model.args.loss_names or "itc" in model.args.loss_names or "tal" in model.args.loss_names or model.circle_task or "cmpm" in model.args.loss_names or "wtriplet" in model.args.loss_names:
			qfeats, gfeats, qids, gids = self._compute_embedding(model)
			qfeats = F.normalize(qfeats, p=2, dim=1)  # text features
			gfeats = F.normalize(gfeats, p=2, dim=1)  # image features
			similarity = qfeats @ gfeats.t()
